...
def meczgrupowy(grupa, a, b):
    f.write(grupa[a][0]+';'+grupa[b][0]+';')
    rankinga=grupa[a][1]
    rankingb=grupa[b][1]
    f.write(str(rankinga)+';'+str(rankingb)+';')
    ...
    if wynik[0]==wynik[1]:
        grupa[b][3] += 1
        grupa[a][3] += 1
    f.write(str(wynik[0])+';'+str(wynik[1])+'\n')
    return wynik

def meczpucharowy(faza, a, b, nastepnafaza, host):
    f.write(faza[a][0] + ';' + faza[b][0] + ';')
    rankinga=faza[a][1]
    rankingb=faza[b][1]
    f.write(str(rankinga)+';'+str(rankingb)+';')
    if host==faza[a][0]:
    ...
    if wynik[0]!=wynik[1]:
        ...
        return wynik
    if wynik[0]==wynik[1]:
        f.write(str(wynik[0]) + ';' + str(wynik[1]) + ';' + str(karniaki[0]) + ';' + str(karniaki[1]) + '\n')
...
def grupka(groupa):
    grupa(groupa)
    #print(tabela(groupa))
    hu=list(tabela(groupa)[2])
    trzeci.append(hu)
    jednaosma.append(tabela(groupa)[0])
    jednaosma.append(tabela(groupa)[1])
